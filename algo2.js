...
var minMax = (array) => {
    ...
    let max = array[0];
    const newArray=[];
    //solution with reduce method instead of for()mthd
    /*
    max = array.reduce((min, p) => p > min ? p : min, array[0]);
    min = array.reduce((min, p) => p < min ? p : min, array[0]);
    */
  ...
  const myArray=[56, 7, 63, 9, 7, 12, 85];
